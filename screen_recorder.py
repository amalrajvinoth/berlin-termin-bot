...
def validate_video_creation(output_file, temp_location=None):
    """
...
def create_image_from_bytes(bytes_obj, root, file_name, extension="png"):
    """
...
def img_path_list_to_cv2_img_list(imgs):
    """
...
class ScreenRecord:
            ...
            if cleanup:
                ...
                if hasattr(self, "imgs"):
                    if self.imgs:
                        ...
                        validate_video_creation(current_file, temp_location)
                        delattr(self, "imgs")
            else:
    ...
    def imgs_to_file_list(self, imgs, temp_location):
        ...
        for idx, img in enumerate(imgs):
            img_path = create_image_from_bytes(img, temp_location, idx)
    ...
    def write_file_list_to_video_file(self, files, height=None, width=None, output_file=None, overwrite=True,
        ...
        if height is None or width is None:
            try:
                ...
                try:
                    ...
                    logger.error("Could not determine video resolution, exiting function...", e)
                    return None
        ...
        for idx, file in enumerate(progress_bar(files, prefix="Progress:", suffix="Complete", length=50)):
            try:
                try:
                    if temp_location:
                        img_path = create_image_from_bytes(file, temp_location, idx)
                        img_obj = cv2.imread(img_path)
                        out.write(img_obj)
                except Exception:
    ...
    def create_video_from_img_folder(self, img_folder, output_file, temp_location=None):
        ...
        if list_of_files:
            ...
            validate_video_creation(output_file, temp_location)
